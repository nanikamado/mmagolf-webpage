            ...
            .then(response =>
                ...
                $$('.rankings>.ranking>tbody').forEach((tbody, i) => {
                    ranking[i].forEach((r, j) => {
                        ...
                        tr.innerHTML = `<td>${j + 1}</td><td>${r[0]} B</td><td>${r[1]}</td><td>${r[2]}</td><td><a href="javascript:void(0)" class="detail-button">コード</a></td>`;
                        tbody.appendChild(tr);
                        let code = document.createElement("tr");
                        code.classList.add('code');
                        code.innerHTML = `<td colspan="${r.length + 1}"><pre>${r[3]}</pre></td>`;
